...
- マウス操作だけで全手番を完結
- 2-4人の同じ端末での対面プレイを優先
- 公式画像や商標素材は使わず、色・数字・アイコン風の自作UIで表現する
...
購入できない場合は、`購入` ボタンを無効化し、足りない色を表示する。

### トークン表示

トークン置き場は色ごとの丸いボタンにする。各ボタンに残り枚数を表示する。
...
- 異なる色を最大3枚
- 同じ色2枚は、その色が取る前に4枚以上ある場合のみ
- 黄金は選択不可
- 場の残数を超えて選択不可

### カードを予約する

1. 公開カード、または山札をクリックする。
2. `予約` を押す。
3. 予約上限が3枚未満なら予約する。
4. 黄金トークンが残っていれば1枚得る。
5. 公開カードの場合は山札から補充する。
...
山札クリック時は、どのカードか見えないまま山札の一番上を予約する。
...
支払いは、まずボーナスを差し引き、不足分を通常トークンで払い、さらに不足した色を黄金で補う。黄金の使い方が複数ある場合だけ、プレイヤーに選ばせる。
...
2. `normal`: 得点カード、貴族条件、ボーナス色の不足を評価する。
