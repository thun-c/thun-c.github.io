...
カード・紋章データは `data/cards.json` と `data/nobles.json` に分け、起動時に `fetch()` で読み込む。GitHub Pages上ではHTTPS配信になるため、ユーザー側はローカルファイル制限を意識しなくてよい。
...
+----------------------+-------------------+---------------+
| 紋章タイル            | 発展カードギルド     | マナ置き場 |
|                      | Level 3: 4枚       | 光 雷 精 炎 闇 |
|                      | Level 2: 4枚       | 全           |
|                      | Level 1: 4枚       |               |
+----------------------+-------------------+---------------+
| 現在プレイヤー詳細: 点数 / 兵 / マナマナ / 予約 |
+----------------------------------------------------------+
| 他プレイヤー概要                                         |
+----------------------------------------------------------+
| 操作ログ                                                 |
...
マナ置き場は種類ごとの丸いボタンにする。各ボタンに残り枚数を表示する。
...
宝石マナをクリックすると「取る候補」に入る。候補が合法手になると `確定` ボタンを有効化する。

全マナは直接クリックで取れない。カード予約時だけ自動で取る。
...
支払いは、まず兵を差し引き、不足分を通常マナで払い、さらに不足した種類を全で補う。全の使い方が複数ある場合だけ、プレイヤーに選ばせる。

### マナ返却

手番終了時にマナが11枚以上なら返却モードに入る。

1. 自分のマナマナをクリックして返却候補に入れる。
2. 合計10枚以下になると `返却して続行` が有効になる。
3. 確定後、紋章判定またはターン終了へ進む。

### 紋章獲得

条件を満たした紋章が1枚なら自動獲得する。

複数ある場合は紋章選択モードに入り、獲得する1枚をクリックして選ぶ。
...
この形にしておくと、CPUの強さを変えても、購入・予約・紋章・終了判定などのルール処理を作り直さずに済む。
...
2. `normal`: 得点カード、紋章条件、兵種類の不足を評価する。
...
購入後の紋章選択や、10枚超過時の返却もCPUに選ばせる。そのため、CPUは通常手番だけでなく `discard` や `noble` フェーズの合法手も選べる必要がある。
...
| `action` | 通常手番 | マナ選択、カード購入、カード予約 |
| `payment` | 購入確認 | 全の割り当て、購入確定 |
| `discard` | マナ返却 | 返却マナ選択 |
| `noble` | 紋章選択 | 獲得する紋章を選択 |
...
- カード、マナ、紋章は固定サイズにしてクリックしやすくする。
- 見た目だけに依存しないよう、マナには文字ラベルも入れる。
- 現在プレイヤーを明確に表示する。
- 購入可能、予約可能、選択中、無効状態を視覚的に分ける。
- 公式アートは使わず、CSSだけでカードとマナを描く。
